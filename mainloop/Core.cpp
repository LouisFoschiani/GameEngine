...
#include "GameObjectFactory.h"
	...
	struct Application
	{
		...
		static EngineContext& CreateContext()
		{
			...
			static EngineContext context(*clock, *input, *engine, *sceneManager);
			return context;
		}
		...
		void Run()
		{
			...
			while (!m_needToQuit)
			{
				...
				if (m_context.Input().AddSceneButtonPressed) {
					SceneManager::CreateScene(1024, "CustomScene");
				}

				if (m_context.Input().LoadSceneButtonPressed) {

					SceneManager::LoadScene("C:\\Users\\jejar\\Desktop\\CustomScene.json");
				}

				if (m_context.Input().AddGameObjectButtonPressed) {
					GameObjectFactory::CreateGameObject("", SceneManager::activeScene);
				}
